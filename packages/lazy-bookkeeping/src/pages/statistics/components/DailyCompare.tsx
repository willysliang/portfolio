 ...
 * @ Modified time: 2023-03-15 16:52:39
...
const DailyCompare = ({ date, type }: Props) => {
  const getData = async () => {
    try {
      ...
      res.sort(
        (a, b) =>
          Number.parseInt(dayjs(a.date).format('MMDD')) -
          Number.parseInt(dayjs(b.date).format('MMDD')),
      )
      const x = res.map((i) => dayjs(i.date).format('MM.DD'))
      const y = res.map((i) => Number(i.total).toFixed(2))
